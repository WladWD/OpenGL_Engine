	...
	out vec4 LightColorDiffuseResult, out vec4 LightColorSpecularResult)
{
	...
	if (gLightDistance < fRad) 
	{
		...
		vec4 mColorD = color * fFalloff * max(dot(N, L), 0.0f);

#ifdef MAIN_VAR
		vec3 brdf = BRDF(N, H, V, L, fRoughness, F0);
		
		vec3 kS = vec3(0.0f);//fresnelSchlick(max(dot(H, V), 0.0f), F0);
		vec3 kD = vec3(1.0f) - kS;
		kD *= 1.0f - fMetallic;
		//kS = vec3(1.0f) - kD;

		LightColorDiffuseResult = vec4(kD, 1.0f) * mColorD;// *mIrRadiance;
		LightColorSpecularResult = vec4(brdf, 1.0f) * mColorD * PI;//;*textureLod(gEnvMap, reflect(-V, N), 4.0 * (fRoughness)) * PI;//;
		...
		vec3 Kspec = Specular(N, H, V, L, fRoughness, F0) * textureLod(gEnvMap, reflect(-V, N), 4.0 * (fRoughness)).rgb * PI;// *mEnvColor.rgb;  reflect(-V, N)
		...
		LightColorDiffuseResult = vec4(dielectric, 1.0f) * mColorD;
		LightColorSpecularResult = vec4(metal, 1.0f) * mColorD;
#endif
...
void main()
{
	...
	*/
		...
		vec4 ResultColor;
#ifdef MAIN_VAR
		vec4 ResultDiffuseAndAmbient = DiffuseTex * DiffuseAndAmbient + Ambient;
