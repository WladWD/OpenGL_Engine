...
ComputeTileShading::LightCompute::LightCompute(LightManager *light_manager, MCamera::Camera *camera, RenderResource::RenderResources<2, 1> *render_resources,
	const DeferredRendering::GBufferData &gbuffer_data, GLuint EnvMap) :
	camera(camera), render_resources(render_resources), gbuffer_data(gbuffer_data), light_manager(light_manager), EnvMap(EnvMap)
{
...
void ComputeTileShading::LightCompute::Draw(const Atmosphere::MoonLightingParam &light_moon, const Atmosphere::GlobalLightingParam &light_sun)
{
	...
	shader_tile_shading->SetPBSTexture();
	glBindTexture(GL_TEXTURE_2D, gbuffer_data.g_buffer_textures[3]);
	...
	shader_tile_shading->SetEnvCubeMap();
