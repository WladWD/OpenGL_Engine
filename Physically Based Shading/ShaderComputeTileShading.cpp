...
void Shader::ShaderComputeTileShading::InitVariable(GLuint program)
{
	...
	g_PBR[current_program] = glGetUniformLocation(program, to_shader_name(g_PBR));
	...
	gEnvMap[current_program] = glGetUniformLocation(program, to_shader_name(gEnvMap));
	/*vec3 direction;
	vec4 diffuse;
	vec4 specular;
	const int NUM = 4;
...
void Shader::ShaderComputeTileShading::SetEnvCubeMap(void)
{
	correct_programm
		glActiveTexture(GL_TEXTURE0 + gEnvMapSlot);
}
...
void Shader::ShaderComputeTileShading::SetPBSTexture(void)
{
	correct_programm
		glActiveTexture(GL_TEXTURE0 + g_PBR_slot);
}
...
void Shader::ShaderComputeTileShading::BindTextureToUnit(void)
{
	...
	mcheck(g_PBR) glUniform1i(g_PBR[current_program], g_PBR_slot);
	...
	mcheck(gEnvMap) glUniform1i(gEnvMap[current_program], gEnvMapSlot);
}
