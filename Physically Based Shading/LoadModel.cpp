...
MEngine::LoadModel::LoadModel(const char *file_name) : w(10), h(10), mInstanceCount(w * h), mOffsetScale(350.0f)
{
	...
	std::string mainFile = file_name;
	std::string FullPath = mainFile.substr(0, mainFile.find_last_of('\\')) + "\\textures\\Texture1.dds";
	EnvMap = load_texture->LoadResource(FullPath);
...
void MEngine::LoadModel::InitScene(const aiScene* pScene, const char *file_name)
{
	...
	mresource->mMaterial.resize(pScene->mNumMaterials * (mInstanceCount + 1));

	//Ns - коэффициет отражения для бликов
	//Ni - коеффициет преломлени
	for (register uint32_t j = 0, counter = 0; j <= w; j++)
	{
		for (register uint32_t k = 0; k < h; k++)
		{
			for (int i = 0; i < pScene->mNumMaterials; i++)
			{
				...
				*/
				...
				mCurentMaterial.material.mF0 = 0.04f;
				mCurentMaterial.material.mMetallic = static_cast<float>(j) / (static_cast<float>(w - 1));// glm::min(static_cast<float>(j) / (static_cast<float>(w - 1) + 0.001f) + 0.0001f, 0.99999f);;
				mCurentMaterial.material.mRoughness = glm::min(0.9f*(static_cast<float>(k) / (static_cast<float>(h-1))) + 0.1f, 1.0f); // 0.6f* 0.4001f

				if (j == w) { mCurentMaterial.material.mF0 = mCurentMaterial.material.mRoughness = mCurentMaterial.material.mMetallic = 0.02f; }
				//mresource->mMaterial.push_back(mCurentMaterial);
				mresource->mMaterial[counter++] = mCurentMaterial;
				if (j == w) { goto next1; }
			}
		}
	}
...
next2:
	...
	for (register uint32_t lp = 0, hl = 0; hl <= w; hl++)
	{
		for (register uint32_t gp = 0; gp < h; gp++)
		{
			for ( i = 0; i < pScene->mNumMeshes; i++)
			{
				if (pScene->mMeshes[i]->HasTextureCoords(0))
				{
					uint32_t mIndex = lp * pScene->mNumMaterials + pScene->mMeshes[i]->mMaterialIndex;
					if (mresource->mMaterial[mIndex].bump_map_id) // pScene->mMeshes[i]->mMaterialIndex * mInstanceCount
					{
						...
						VNTT_object->mEntries[k].SetMaterialID(mIndex); //paiMesh->mMaterialIndex * mInstanceCount

						InitMeshVNTT(base_vertex_VNTT, base_index_VNTT, paiMesh,
							mOffsetScale * (static_cast<float>(hl) / static_cast<float>(w)), mOffsetScale * (static_cast<float>(gp) / static_cast<float>(w)));

						base_vertex_VNTT += paiMesh->mNumVertices;
						base_index_VNTT += paiMesh->mNumFaces * 3;

						MeshLoad::ObjectMaterial material = mresource->mMaterial[mIndex];
					...
					else
					{
						...
						VNT_object->mEntries[j].SetMaterialID(mIndex);
...
void MEngine::LoadModel::InitMeshVNT(uint32_t base_vertex, uint32_t base_index, const aiMesh *paiMesh, float xOffset, float zOffset, float scale)
{
	...
	if (scale != 1.0f) { xOffset = zOffset = mOffsetScale * 0.5f; }
	...
	for (register uint32_t i = 0; i < paiMesh->mNumVertices; i++) 
	{
		...
		if (scale == 1.0f)mesh_VNT.m_vertex[base_vertex + i].l_pos = glm::vec3(vert_pos.x + xOffset, vert_pos.y, vert_pos.z + zOffset) * scale;
		else 
		{
			mesh_VNT.m_vertex[base_vertex + i].l_pos = glm::vec3(vert_pos.x , vert_pos.y, vert_pos.z) * scale;
			mesh_VNT.m_vertex[base_vertex + i].l_pos += vec3(xOffset, 0.0f, zOffset);
		}
...
void MEngine::LoadModel::InitMeshVNTT(uint32_t base_vertex, uint32_t base_index, const aiMesh *paiMesh, float xOffset, float zOffset)
{
	//здесь собираеться 1 обьект, так что при необходимости можно его выделить отсюда в отдельный буффер(например если обьект динамичен), (для скелетной анимации)
	...
	for (register uint32_t i = 0; i < paiMesh->mNumVertices; i++)
	{
		...
		mesh_VNTT.m_vertex[base_vertex + i].l_pos = glm::vec3(vert_pos.x + xOffset, vert_pos.y, vert_pos.z + zOffset);
...
void MEngine::LoadModel::MCreateBufferVNTT(void) 
{
	...
	uint32_t index_count = mesh_VNTT.m_index.size();
	uint32_t vertex_count = mesh_VNTT.m_vertex.size();
...
uint32_t MEngine::LoadModel::GetEnvTexture(void)
{
	return EnvMap;
}
