...
void MCamera::Camera::UpdateCamera(float delta_time_ms)
{
	...
	vec3 y_vec = glm::mat3_cast(x_quat)[1];// * vec3(0.0f, 1.0f, 0.0f);
	...
	rotation_matrix = glm::transpose(rotation_matrix);///почему????????
	...
	view = glm::mat4_cast(res_quat) * mat4(st0, st1, st2, st3);//почему????????

	ProjectView = project * view;// *glm::inverse(glm::translate(glm::mat4(1.0f), glm::vec3(1000.0f, 0.0f, 0.0f)) * glm::rotate(glm::mat4(1.0f), 1.7f, glm::vec3(0.0f, 1.0f, 0.0f)));

	dx = dy = dz = 0.0f;
}

mat4 MCamera::Camera::GetViewMatrix() 
{
	return view;
}

mat4 MCamera::Camera::GetProjMatrix() 
{
	return project;
}

mat4 MCamera::Camera::GetProjViewMatrix() 
{
	return ProjectView;
}
...
vec3 MCamera::Camera::GetPosition() 
{
	return position;
}

vec3 MCamera::Camera::GetCamDir() 
{
	return -k;
}
...
uint32_t MCamera::Camera::GetWeight(void) 
{
	return width;
}

uint32_t MCamera::Camera::GetHeight(void) 
{
	return height;
}