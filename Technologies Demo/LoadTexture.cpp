...
Resource::GLTextureResource Resource::LoadTexture::LoadTexture2DArrayFromFileDirectXTex(string texture_name, uint32_t max_count)
{
	...
	while (name_x < max_count) 
	{
		...
		if (!init) 
		{
			pixels = new uint8_t[image.GetPixelsSize() * max_count];//height*width*max_count*bpp];
			init = true;
		}
		memcpy(pixels + image.GetPixelsSize() * name_x, image.GetImages()->pixels, image.GetPixelsSize());// height*width * sizeof(uint8_t)*bpp);
		// + height*width*name_x*bpp
